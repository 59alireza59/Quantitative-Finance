...
## AXP prices and log-returns paths
par(mfrow=c(1,2))
...
## Plot of AXP returns density distribution versus the Gaussian distribution
plot(density(X))
d <- function(x) dnorm(x, mean=mu.hat, sd=sigma.hat)
curve(d, -.2, .2, col="red",add=TRUE, n=500)


## Plot of AXP returns density distribution versus the different distributions
par(mfrow=c(2,2))
par(mar=c(3,3,3,1))
grid <- NULL
...
## Plot of AXP returns density distribution versus the NIG distribution
nigFit(X,trace=FALSE)
...
n # 148
S1 <- S[1:(n*0.85)]
#splitting the time series, taking the first ...%
m <- length(S1)
m # 125
S.true <- S[-(1:(m-1))]
S.true
...
X <- as.numeric(X)
X
mod.NIG <- nigFit(X,trace=FALSE)@fit$par
#mod.NIG is a vector of number
alpha   <- mod.NIG["alpha"]
beta    <- mod.NIG["beta"]
delta   <- mod.NIG["delta"]
mu      <- mod.NIG["mu"]
...
## Plot of AXP returns density distribution versus the NIG distribution
plot(density(X)) 
#black= distr of returns
lines(xvals,yvals.NIG,col="red",lty=5,lwd=2)
#red= distr of NIG
lines(xvals, dnorm(xvals, mean=mean(X), sd=sd(X)), col="blue", lty=3,lwd=2)
#blue= GBM distribution

S0 <- S1[m]
#terminal value of the first part of data
S0 # 95.64
...
SS <- S0*exp(Xt)
## This gives the trajectory of the model
A <- c(S1, S.true)

## AXP simulated pattern under Lévy process (red line)
## and real pattern (blue line).

plot(A,type="n")
points(S1,type="l")
points(m+1:length(SS) , SS,type="l",col="red") 
#simulated data (trajectory)
...
## Lévy payoff (red line), GBM payoff (blue line)
## with respect to the null real payoff.
plot(density(levy.price),xlim=c(-5,10), main="Density Levy Price")
abline(v=f(S[length(S)]))
#black true payoff trajectory
abline(v=p,col="red")
#red levy payoff
abline(v=p0,col="blue")
#blue B&S payoff
#these are just two trajectories
################################
